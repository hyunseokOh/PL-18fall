...
struct
  ...
  let op2fn =
    ...
    | EQ -> (fun (v1,v2) -> match (v1,v2) with
                            ...
                            | (Loc n1, Loc n2)
                            | (Int n1, Loc n2)
                            | (Loc n1, Int n2) -> if (n1=n2) then Bool true else Bool false
  ...
  let rec eval env mem exp = 
    ...
    | APP (e1, e2) ->
      ...
      | RecFun (f, x, e) -> eval ((env' @+ (f, v1)) @+ (x,v2)) m'' e                
      )  
    | IF (e1, e2, e3) ->
      let (v1, m') = eval env mem e1 in
    ...
    | LET (d,e2) -> (match d with
                   | REC (f,x,e1) -> 
                           let (v1,mem') = (Closure(RecFun (f,x,e1),env),mem) in
                           eval (env @+ (f,v1)) mem' e2
                   | VAL (x,e1) ->
                           let (v1,mem')=eval env mem e1 in
                           eval (env @+ (x,v1)) mem' e2
                   )
    ...
    | ASSIGN (e1,e2) -> 
      let (l,mem') = eval env mem e1 in
      let lval=match l with
              | Int ll -> ll
              | Loc ll -> ll
              | _ -> raise (RunError ("Assign")) in

      let (v,mem'') = eval env mem' e2 in
      let mf = store mem'' (lval,v) in
      (v, mf)

    | BANG e ->
      let (l,mem') = eval env mem e in
      let lval=match l with 
              | Int ll -> ll
              | Loc ll -> ll
              | _ -> raise (RunError ("Bang")) in
      let v = load mem' (lval) in
      (v, mem')
