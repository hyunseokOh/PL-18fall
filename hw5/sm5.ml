  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then (
      ...
      let rec searchE e = match e with 
                     ...
                     | (_,Loc l)::tl -> (
                             (chain l)@(searchE tl))
                     | (_,Proc _)::tl -> searchE tl
      in
      
      let rec searchK k = match k with
                         | [] -> []
                         | (c,e)::tl -> searchE e @ searchK tl
      in
      
      let rec searchS s = match s with
                         | [] -> []
                         | V v::tl -> (match v with
                                   | L l -> chain l @ searchS tl
                                   | _ -> searchS tl
                                  )
                         | _::tl -> searchS tl
      in

      let _ = reachable_locs := (searchE e @ searchK k @ searchS s) in
